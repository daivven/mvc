 ...
 *  Author     : 疯狂秀才(1055818239)
 *  Email      : hxl_apple@163.com
 *  Blog       : http://hxling.cnblogs.com
 *  
 *  使用方法：
...
; (function ($) {
    ...
    function loadingInit(target, options) {
        var $mask = $('<div class="xiucai-qqmsg-mask"></div>'); //遮罩层
        var $loading = $('<div class="xiucai-qqmsg-layer_wrap"/>'); //消息
        ...
        if (options.type && options.type != 'loading' ) {
            $loading.appendTo(target).attr("id", layerId + '_loading').empty()
                .append('<span class="xiucai-qqmsg-layer"  id="mode_tips_v2"><span class="' + iconCls + '"></span>' +
                    options.msg + '<span class="gtl_end"></span></span>').css('z-index', options.zIndex + 1);
        } else {
            $loading.appendTo(target).attr("id", layerId + '_loading').empty()
            .append('<span class="xiucai-qqmsg-layer"  id="mode_tips_v2"><span class="gtl_ico_clear"></span><span class="' + iconCls + '"></span>' +
                options.msg + '<span class="gtl_end"></span></span>').css('z-index', options.zIndex + 1);
        }
    ...
    $.fn.hLoading.hide = function(jq,fn) {
        return jq.each(function() {
            var wrap = $(this);
            $("div.xiucai-qqmsg-mask", wrap).fadeOut(function() {
                $(this).remove();
            });
            $("div.xiucai-qqmsg-layer_wrap", wrap).fadeOut(function() {
                $(this).remove();
            });
