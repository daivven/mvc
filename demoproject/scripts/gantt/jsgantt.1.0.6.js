        ...
        var vMaxDate = new Date(); var vMinDate = new Date(); var vTmpDate = new Date(); var vNxtDate = new Date(); var vCurrDate = new Date(); var vTaskLeft = 0; var vTaskRight = 0; var vWeekBack = ''; var vNumCols = 0; var vID = 0; var vMainTable = ""; var vLeftTable = ""; var vRightTable = ""; var vDateRowStr = ""; var vItemRowStr = ""; var vColWidth = 0; var vColUnit = 0; var vChartWidth = 0; var vNumDays = 0; var vDayWidth = 0; var vStr = ""; var vNameWidth = 220; var vStatusWidth = 70; var vLeftWidth = 15 + 220 + 70 + 70 + 70 + 70 + 70; var gantthtmlstrarr = []; if (vTaskList.length > 0) {
            ...
            gantthtmlstrarr.push('<div region="west" split="true"  id=leftside><table cellSpacing=0 cellPadding=0 id="leftTable" class="leftTable" width=' + vLeftWidth + '><tbody><tr class="left-table-top"><td style="width: 15px;">&nbsp;</td><td colspan="' + topColspan + '" class="projectame">' + pProjectName + '</td></tr>'); gantthtmlstrarr.push('<tr class="caption"><td style="width: 15px; height: 20px">&nbsp;</td><td style="width: ' + vNameWidth + 'px; height: 20px">任务名称</td>'); if (vShowRes == 1)
                ...
                gantthtmlstrarr.push('<span style="cursor:pointer"><a class="taskname" href="#bardiv_' + vID + '">' + task.getName() + '</a></span></td>'); if (vShowRes == 1)
                    ...
                    gantthtmlstrarr.push('  <td class=gname style="width: 60px;" align=center>' + $D(task.getStart()).strftime(vDateDisplayFormat) + '</td>'); if (vShowEndDate == 1)
                    gantthtmlstrarr.push('  <td class=gname style="width: 60px;" align=center>' + $D(task.getEnd()).strftime(vDateDisplayFormat) + '</td>'); gantthtmlstrarr.push('</tr>');
            }
            ...
            gantthtmlstrarr.push("</tr></tbody></table>"); for (i = 0; i < vTaskList.length; i++) {
                vTmpDate.setFullYear(vMinDate.getFullYear(), vMinDate.getMonth(), vMinDate.getDate()); vTaskStart = vTaskList[i].getStart(); vTaskEnd = vTaskList[i].getEnd(); vNumCols = 0; vID = vTaskList[i].getID(); vNumUnits = (vTaskEnd - vTaskStart) / (24 * 60 * 60 * 1000) + 1; if (vTaskList[i].getVisible() == 0)
                    ...
                    var keyline = drawKeyLine(vMinDate, vDayWidth); gantthtmlstrarr.push(keyline); if (vTaskList[i].getGroup()) { gantthtmlstrarr.push('<div id=childrow_' + vID + ' class="rightTable-TaskItem groupbg" style="width: ' + vChartWidth + 'px;" >' + vItemRowStr + '</div>'); gantthtmlstrarr.push(vWeekBack + '<div id=bardiv_' + vID + ' style="position:absolute; top:5px; left:' + vTaskLeft + 'px; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px">' + '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:#000000; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px;  cursor: pointer;opacity:0.9;">' + '<div style="Z-INDEX: -4; float:left; background-color:#F0F0F0; height:3px; overflow: hidden; margin-top:1px; ' + 'margin-left:1px; margin-right:1px; filter: alpha(opacity=80); opacity:0.8; width:' + vTaskList[i].getCompStr() + '; ' + 'cursor: pointer;" onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '",300,200);>' + '</div>' + '</div>' + '<div style="Z-INDEX: -4; float:left; background-color:#000000; height:4px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:right; background-color:#000000; height:4px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:left; background-color:#000000; height:3px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:right; background-color:#000000; height:3px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:left; background-color:#000000; height:2px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:right; background-color:#000000; height:2px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:left; background-color:#000000; height:1px; overflow: hidden; width:1px;"></div>' + '<div style="Z-INDEX: -4; float:right; background-color:#000000; height:1px; overflow: hidden; width:1px;"></div>'); } else { gantthtmlstrarr.push('<div id=childrow_' + vID + ' class="rightTable-TaskItem" style="width: ' + vChartWidth + 'px;" >' + vItemRowStr + '</div>'); gantthtmlstrarr.push(vWeekBack + '<div id=bardiv_' + vID + ' style="position:absolute; top:4px; left:' + vTaskLeft + 'px; height:18px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px">' + '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:#' + vTaskList[i].getColor() + '; height: 13px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px; cursor: pointer;opacity:0.9;" ' + '>' + '<div class=gcomplete style="Z-INDEX: -4; float:left; background-color:black; height:5px; overflow: auto; margin-top:4px; filter: alpha(opacity=40); opacity:0.4; width:' + vTaskList[i].getCompStr() + '; overflow:hidden">' + '</div>' + '</div>'); }
                    if (g.getCaptionType()) {
                        vCaptionStr = ''; switch (g.getCaptionType()) { case 'Caption': vCaptionStr = vTaskList[i].getCaption(); break; case 'Resource': vCaptionStr = vTaskList[i].getResource(); break; case 'Duration': vCaptionStr = vTaskList[i].getDuration(vFormat); break; case 'Complete': vCaptionStr = vTaskList[i].getCompStr(); break; }
